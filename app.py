...
if st.session_state.slots is not None:
    ...
    if st.button("Proses Video Hasil Deteksi") or st.session_state.video_process is not None:
        ...
        if st.session_state.video_process is None:
            ...
            tracker = Tracker(detector[1].feature_model)
        ...
        cap = cv2.VideoCapture(process["video_path"])
        cap.set(cv2.CAP_PROP_POS_FRAMES, process["frame_idx"])
        out = cv2.VideoWriter(
            process["temp_video_path"],
            cv2.VideoWriter_fourcc(*"mp4v"),
            process["fps"],
        ...
        slots = process["slots"]
        DETECT_EVERY_N = 5
        last_detections = process["last_detections"]
        last_results = process["last_results"]

        BATCH_SIZE = 50  # Proses 50 frame per batch
        processed = 0
        progress_bar = st.progress(min(frame_idx / total_frames, 1.0))
        preview_placeholder = st.empty()

        while processed < BATCH_SIZE and frame_idx < total_frames:
            ret, frame = cap.read()
            if not ret:
                break

            if frame_idx % DETECT_EVERY_N == 0:
                last_results = detector(frame)[0]
                last_detections = []
        ...
        if frame_idx >= total_frames:
            ...
            st.info(f"Proses batch selesai. {frame_idx}/{total_frames} frame telah diproses.")
